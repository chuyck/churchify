...
and function_body_to_expression expr body =
  match body with
  | Pfunction_body e -> e
  | Pfunction_cases cases ->
      {
        pexp_desc = Pexp_function_cases cases;
    ...
    | param :: rest ->
      ...
      let inner_expr =
             if rest = [] then
               ...
               translate_expression (function_body_to_expression expr body)
             else
               (* recurse for remaining parameters *)
               translate_expression { expr with
                                      pexp_desc = Pexp_function (rest, _closed_flag, body) }
           in
           Lam(name, inner_expr)))
